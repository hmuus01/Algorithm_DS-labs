...
public class Graph {
    ...
    public int SPCost(String from, String to) {
        ...
        while (!testingVertex.isEmpty()){
            ...
            for(int i =0; i< interimV.adjlist.size();i++)
            {
                ...
                //(interimV.adjlist.get(i).to.distanceFromPrev)>(storeInterimDist +(double) interimV.adjlist.get(i).weight))
                if((storeInterimDist + (double) interimV.adjlist.get(i).weight) < (interimV.adjlist.get(i).to.distanceFromPrev))
                {
    ...
    public double calCulateDistanceFromEdge(Vertex to)
    {
        return (int) (to.distanceFromPrev - to.pastVertex.distanceFromPrev);
    }
    ...
    public Graph SP(String from, String to) {
        ...
        ArrayList<Vertex> checkedV = new ArrayList<Vertex>();
        Vertex current = getVertex(to);
        g.addVertex(current.getName());
        checkedV.add(current);
        while(!current.getName().equals(getVertex(from).getName())){
            g.addVertex(current.pastVertex.name);
            g.addEdge(current.name,current.pastVertex.name,(int) calCulateDistanceFromEdge(current));
                //(int)(double)(current.distanceFromPrev() - current.pastVertex.distanceFromPrev()));
            // ArrayList<Edge> adjacentEdges = current.adjlist;
            // int shortest = Integer.MAX_VALUE;
            // Vertex next = null;
            // Edge nextEdge = null;
            // for(Edge edge: adjacentEdges) {
            //     if(edge.weight < shortest && !checkedV.contains(edge.to)) {
            ...
            // g.addVertex(next.name);
            // g.addEdge(nextEdge.from.name, nextEdge.to.name, shortest);
            //  checkedV.add(next);
            current = current.pastVertex;
            //checkedV.add(current);
        }
//      g.addVertex(interimV.adjlist.get(i).to.getName());
//      Edge current = this.getEdge(previousVertex.getName(), interimV.adjlist.get(i).to.getName());
//      g.addEdge(previousVertex.getName(), interimV.adjlist.get(i).to.getName(), current.getWeight());
//      previousVertex = interimV.adjlist.get(i).to;
   //      Graph g = new Graph();
   // if(from == null|| to == null)
   //      {
   //          return this;
   ...
   //                  interimV.adjlist.get(i).to.distanceFromPrev = interimV.distanceFromPrev + (double) interimV.adjlist.get(i).weight;
   //                  interimV.adjlist.get(i).to.pastVertex = interimV;
   //                  g.addVertex(interimV.name);
   //                  g.addEdge(interimV.adjlist.get(i).from.name,interimV.adjlist.get(i).to.name,interimV.adjlist.get(i).weight);
